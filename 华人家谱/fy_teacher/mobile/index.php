...
$setting = pdo_fetch("SELECT vipdiscount,teacher_income,qiniu FROM " .tablename($this->table_setting). " WHERE uniacid=:uniacid LIMIT 1", array(':uniacid'=>$uniacid));
...
if($op=='display'){
	...
	if($id>0){
		$lesson_income = $lesson['teacher_income'];
	}else{
		$lesson_income = $lesson['teacher_income']?$lesson['teacher_income']:$setting['teacher_income'];
	}
	...
	if(checksubmit('submit')){
		...
		$data['teacher_income']	= $lesson['teacher_income']?$lesson['teacher_income']:$setting['teacher_income'];
