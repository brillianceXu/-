 ...
 */
 ...
 /* 检查显示折扣活动是否过期 */
 $discount_list = pdo_fetchall("SELECT * FROM " .tablename($this->table_discount). " WHERE uniacid=:uniacid AND endtime<:endtime LIMIT 5000", array(':uniacid'=>$uniacid,':endtime'=>time()));
 foreach($discount_list as $item){
	pdo_update($this->table_discount, array('status'=>0), array('discount_id'=>$item['discount_id']));
 }
